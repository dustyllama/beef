...
new_wait = '''wait_for_media_ready() {
  ...
  for _ in $(seq 1 45); do
    ...
    # A MediaScanner row can exist for a while with is_pending=1 and null/zero
    # metadata. Launching the app in that window tests Android's scanner, not
    # NeuronTap. Require a finalized, non-empty row before cold-start QA.
...
p.write_text(s)
print("Hardened v0.8.1 smoke for finalized MediaStore rows and live additions")

...
if activity_anchor not in s:
    raise RuntimeError("v8 mainline repair MainActivity manifest anchor missing")
s = s.replace(
    activity_anchor,
...
for old, new in [
    ...
    if old not in s:
        raise RuntimeError(f"v8 mainline repair gallery state anchor missing: {old}")
...
if old not in s:
    raise RuntimeError("v8 mainline repair duration anchor missing")
s = s.replace(old, new, 1)
...
if "var durationMs by remember(item.id) { mutableLongStateOf(1L) }" not in s:
    raise RuntimeError("v8 mainline repair duration state anchor missing")
...
'''
if old_fit not in s:
    raise RuntimeError("v8 mainline repair video fit anchor missing")
...
if old_slider not in s:
    raise RuntimeError("v8 mainline repair slider anchor missing")
...
if old_clock not in s:
    raise RuntimeError("v8 mainline repair clock anchor missing")
s = s.replace(old_clock, new_clock, 1)
p.write_text(s)

print("Applied v0.8.3 mainline video/navigation repair")

...
assert_scrub_updates_frame() {
  # Pause first so any visual change comes from seek rendering, not playback time.
...
# Two sentinels make the origin-tab regression objectively testable: All must
# contain image+video, Videos must contain only the video.
...
# Hide controls and test the pixels actually being rendered.
screen_center_tap
sleep 0.5
assert_landscape_video_not_stretched
screen_center_tap
...
# The sentinel image makes this an actual origin-context test instead of a tab-label guess.
...
echo "NeuronTap v0.8.1 deterministic emulator torture test passed."

...
if start < 0 or end < 0:
    raise RuntimeError("v0.8.1 aspect smoke function anchors missing")
...
# The QA clip is a full-frame colorful test pattern. Small colored controls,
# icons, and text must not be allowed to enlarge the measured video bounds.
def colorful(x, y):
    i = (y * w + x) * 3
    r, g, b = data[i:i+3]
...
row_counts = []
for y in range(h):
    row_counts.append(sum(1 for x in range(w) if colorful(x, y)))

# A real video row is dense across the screen; UI chrome is sparse.
...
col_counts = []
for x in range(w):
    col_counts.append(sum(1 for y in range(best_start, best_end) if colorful(x, y)))
...
print(
    f'isolated video band x={x1}:{x2} y={best_start}:{best_end} '
    f'size={band_w}x{band_h} aspect={ratio:.3f} colorful_coverage={coverage:.3f}'
)

# 640x360 = 1.777...; tolerate decoder/scaling edges, not stretching.
if not (1.60 <= ratio <= 1.95):
    raise SystemExit(
        f'SMOKE FAILURE: isolated 16:9 video band is stretched; measured aspect={ratio:.3f}'
    )
PY
}
'''
...
p.write_text(s)
print("Replaced v0.8.1 aspect smoke with dense video-band measurement")
